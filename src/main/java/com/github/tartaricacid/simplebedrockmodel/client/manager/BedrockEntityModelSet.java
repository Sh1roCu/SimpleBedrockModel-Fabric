...
import cn.sh1rocu.simplebedrockmodel.client.SimpleBedrockModelClient;
    ...
    protected @NotNull Void prepare(@NotNull ResourceManager manager, @NotNull ProfilerFiller filler) {
        ...
        this.knowLocations.keySet().forEach(location -> {
            ...
            manager.getResource(path).ifPresentOrElse(model -> {
                SimpleBedrockModelClient.LOGGER.info("Loading bedrock model file: {}", path);
                try (InputStream stream = model.open()) {
                    this.models.put(location, modelFunction.apply(stream));
                } catch (IOException e) {
                    SimpleBedrockModelClient.LOGGER.error("Failed to load model file: {}", path, e);
                }
            }, () -> SimpleBedrockModelClient.LOGGER.error("Not found model file: {}", path));
        });
        return null;
    }
